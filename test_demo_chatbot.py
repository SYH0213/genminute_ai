"""
발표/데모용 챗봇 자동 테스트 스크립트
demo_questions.md의 질문들을 자동으로 테스트하고 결과를 저장합니다.
"""
...
def extract_questions_from_md(file_path):
    ...
    for match in matches:
        ...
        difficulty = match[3].strip()

        questions.append((question_num, question_text, intent, difficulty))
...
def write_results_to_markdown(results, output_file):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"**성공률**: {success_count/len(results)*100:.1f}%\n\n")
        ...
        for item in results:
            ...
            if result['success']:
                ...
                f.write(f"### ❌ 오류\n\n")
                f.write(f"{result.get('error', '알 수 없는 오류')}\n\n")
        ...
        if success_count == len(results):
            f.write("### 🎉 완벽한 성공!\n\n")
            f.write("모든 질문에 정확한 답변이 생성되었습니다.\n")
            f.write("발표/데모용으로 사용하기에 적합합니다.\n\n")
        else:
            f.write(f"### ⚠️ {len(results) - success_count}개 질문 실패\n\n")
            failed_questions = [r for r in results if not r['result']['success']]
            for failed in failed_questions:
                f.write(f"- Q{failed['question_num']}: {failed['question']}\n")


def main():
    """메인 실행 함수"""
    print("=" * 60)
    print("발표/데모용 챗봇 자동 테스트 시작")
    print("=" * 60)
    ...
    if SESSION_COOKIE == "여기에_세션_쿠키_붙여넣기":
        ...
        print("2. F12 > Network > chat 요청 > Headers > Cookie 값 복사")
        print("3. test_demo_chatbot.py의 SESSION_COOKIE 변수에 붙여넣기")
        return
    ...
    for i, (q_num, question, intent, difficulty) in enumerate(questions, 1):
    ...
    if success_count == len(results):
        print("\n🎉 완벽! 모든 질문이 성공했습니다!")
        print("발표/데모에 사용하기 적합합니다.")
    else:
        print(f"\n⚠️ {len(results) - success_count}개 질문 수정 필요")
